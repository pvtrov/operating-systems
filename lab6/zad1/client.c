...
void receive_message_nowait(int queue_id, message* message_, long mode){
    if (msgrcv(queue_id, message_, sizeof(message) - sizeof(long ), mode, IPC_NOWAIT) == -1){
        printf("cannot receive message");
        exit(1);
    }
}
...
void catch(){
    while(!is_empty(client_queue)){
        message message_;
        receive_message_nowait(client_queue, &message_, 0);
...
int main (){
    ...
    printf("client queue id: %d", client_queue);
